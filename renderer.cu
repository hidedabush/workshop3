...
// Simple test kernel: fill buffer with gradient color (no ray tracing)
__global__ void testKernel(unsigned int* fb, int width, int height)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    // Create a simple gradient: red to blue
    float r = (float)x / (float)width;
    float g = (float)y / (float)height;
    float b = 0.5f;

    vec3 color(r, g, b);
    fb[y * width + x] = packColor(color);
}
                 ...
                 const Camera& cam, const Scene& scene)
{
    ...
    // Launch the test kernel (simple gradient, no ray tracing)
    // <<<gridSize, blockSize>>> specifies the grid and block layout.
    testKernel<<<gridSize, blockSize>>>(d_fb, width, height);
